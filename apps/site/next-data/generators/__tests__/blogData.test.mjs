...
describe('generateBlogData', () => {
  it('should return zero posts and only the default "all" category if no md file is found', async t => {
    files = [];
    ...
    t.mock.module('../../../next.helpers.mjs', {
      namedExports: {
        getMarkdownFiles: () => {
    ...
    // assert.equal(blogData.categories, ['all']);
    // assert.equal(blogData.posts, []);
  });
  ...
  //   assert.equal(post.date, new Date('2020-01-01T00:00:00.000Z'));
  ...
  //   assert.equal(blogData.posts[0].date, new Date('2020-01-01T00:00:00.000Z'));
  //   assert.equal(blogData.posts[0].author, 'author-a');
  //   assert.equal(blogData.posts[1].title, 'POST 2');
  //   assert.equal(blogData.posts[1].date, new Date('2020-01-02T00:00:00.000Z'));
  ...
  //   assert.equal(blogData.categories.sort(), [
  //     'all',
  //     'category-a',
  //     'category-b',
  //     'uncategorized',
  ...
  //   assert.equal(blogData.posts.map(p => p.slug).sort(), [
